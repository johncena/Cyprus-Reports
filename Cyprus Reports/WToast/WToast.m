...
- (void)__animationDidStop:(NSString *)animationID __finished:(NSNumber *)finished __context:(void *)context {
	if ([animationID isEqualToString:@"hide"]) {
        ...
        if ([prefs boolForKey:@"longTime"]==TRUE) {
            NSLog(@"long time");
            [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(__hide) userInfo:nil repeats:NO];
            [prefs setBool:false forKey:@"longTime"];
            [prefs synchronize];
        }
        else
        {
		[NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(__hide) userInfo:nil repeats:NO];
            NSLog(@"Short time");
        }
	}
}
