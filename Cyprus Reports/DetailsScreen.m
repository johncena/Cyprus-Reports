...
-(void)getData{
    ...
    NSString *post =[NSString stringWithFormat:@"catId=%d",_catId];
...
//        [titleLabel release];
        ...
        descriptionLabel.font= [UIFont fontWithName:@"Helvetica-Bold" size:12.0];
...
//    descriptionLabel.textColor= [UIColor colorWithRed:200.0/256.0 green:200.0/256.0 blue:200.0/256.0 alpha:1.0];
    descriptionLabel.textColor= [UIColor whiteColor];
...
-(void) connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    else
    {
        ...
        if (details)
        {
            getDetailArray= [[details objectForKey:@"userDetails"] retain];
        }
                  
    }
    if (getDetailArray.count == 0 ) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No Reports Found" message:@"No reports found under this category for now." delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
        [alert show];
        [alert release];
    }
